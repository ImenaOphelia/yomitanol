...
def save_duplicates_to_file(db_path, table_name, column_name, duplicates):
    ...
    with open("duplicates_report.txt", "w", encoding="utf-8") as file:
        for value, _ in duplicates:
            cursor.execute(
                f"SELECT rowid, * FROM {table_name} WHERE {column_name} = ?", (value,)
            )
...
def delete_duplicates(db_path, table_name, column_name, duplicates, delete_all):
    ...
    for value, _ in duplicates:
        ...
        if delete_all:
            ...
            for row in rows[1:]:
                confirm = (
                    input(f"Do you want to delete this entry? {row} (y/n): ")
                    .strip()
                    .lower()
                )
                if confirm == "y":
                    cursor.execute(
                        f"DELETE FROM {table_name} WHERE rowid = ?", (row[0],)
                    )
...
def main():
    ...
    if duplicates:
        ...
        delete_all = (
            input("Do you want to delete all duplicates at once? (y/n): ")
            .strip()
            .lower()
            == "y"
        )
        ...
        confirm_deletion = (
            input("Are you sure you want to proceed with the deletions? (y/n): ")
            .strip()
            .lower()
        )
